...
function playSongByIndex(index, isAutoSkip=false) {
    ...
    if(uiCover) uiCover.src = song.cover || 'https://q1.qlogo.cn/g?b=qq&nk=3075304265&s=640';
...
async function fetchNeteaseLyric(songId) {
    const proxyUrl=`https://cros.bxyaa.me/?url=${encodeURIComponent('https://music.163.com/api/song/lyric?os=pc&id='+songId+'&kv=-1&tv=-1')}`;
...
async function getRandomMusic() {
    ...
    const proxyUrl='https://cros.bxyaa.me/?url='+encodeURIComponent('https://api.52vmy.cn/api/music/wy/rand');
...
async function loadWeather() {
    ...
    try{
        ...
        const proxyUrl=`https://cros.bxyaa.me/?url=${encodeURIComponent(wUrl)}`;
...
// ================= 播放历史 =================
const HISTORY_KEY = 'bxya_play_history';
let playHistory = [];
...
// ================= 夜间模式 =================
const NIGHT_MODE_KEY = 'bxya_night_mode';
...
// ================= 背景系统 =================
const BG_KEY='bxya_bg', BG_TYPE_KEY='bxya_bg_type', BING_API='https://bing.biturl.top/?resolution=1920&format=json&index=0&mkt=zh-CN', DEFAULT_BG='https://img.8845.top/acg';
...
function initBgLocal(){
    ...
    const ss=localStorage.getItem('bxya_video_scale');if(ss)currentVideoScale=ss;
...
function setVideoScale(s){currentVideoScale=s;applyVideoScale();try{localStorage.setItem('bxya_video_scale',s);}catch(e){}}
...
function redirectToTheme(themeId) {
    ...
    try { localStorage.setItem('bxya_theme', themeId); sessionStorage.setItem('theme_transition','1'); } catch(e) {}
    // 渐进变黑
...
function initThemeRedirect() {
    ...
    const saved = localStorage.getItem('bxya_theme') || 'aurora';
...
function preloadBgThenHide(callback){
    ...
    let bgUrl=null;
    try{bgUrl=localStorage.getItem('bxya_bg');}catch(e){}
    if(!bgUrl)bgUrl='https://img.8845.top/acg';
    // 如果是视频背景，直接隐藏
    try{if(localStorage.getItem('bxya_bg_type')==='video'){callback();return;}}catch(e){}
    const img=new Image();
